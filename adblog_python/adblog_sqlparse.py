...
writelogfields = ['nodename', 'log_time', 'user_name', 'database_name', 'connection_from', 'session_id', 'command_tag']
        ...
        try:
            for row in csvfile:
                ...
                if logtimestr > endtime:
                    continue 
                if (row['command_tag'] in cmd_list) :
                    ...
                    outline = {'nodename':nodename, 'log_time':logtime, 'user_name':username, 'database_name': dbname, 'connection_from':connection_from,'session_id':session_id, 'command_tag':command_tag}
...
def end_process():
    ...
    cre_table = """
create table adblog_sqlparsebind
(
nodename text,    
logtime timestamp,
username text,
...
;
"""
    copy_line = 'then execute "copy adblog_sqlparsebind from \''+outcsvfilename+'\'  delimiter \''+out_delimiter+'\' csv;" to load csv data '
...
def get_logrange_input():
    ...
    outcsvfilename = os.getcwd()+'/adblog_sqlparsebind_'+nodename+'_'+begintime+'_'+endtime+'.csv'
